...
class MappingsPage {
	...
	public function handle_actions(): void {
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] === 'delete' && ! empty( $_GET['mapping_id'] ) ) {
			...
			$this->redirect_with_notice( $redirect_url, __( 'Mapping deleted.', 'images-sync-for-cloudflare' ) );
		}
	...
	private function source_type_labels(): array {
		return array(
			'acf_field'                => __( 'ACF Image Field', 'images-sync-for-cloudflare' ),
			'featured_image'           => __( 'Featured Image (Thumbnail)', 'images-sync-for-cloudflare' ),
			'post_meta_attachment_id'  => __( 'Meta Field → Attachment ID', 'images-sync-for-cloudflare' ),
			'post_meta_url'            => __( 'Meta Field → Image URL', 'images-sync-for-cloudflare' ),
			'attachment_id'            => __( 'Attachment (post is the image)', 'images-sync-for-cloudflare' ),
		);
	}
	...
	public function ajax_test_mapping(): void {
		...
		if ( $post_id <= 0 ) {
			wp_send_json_error( __( 'Please enter a valid post ID.', 'images-sync-for-cloudflare' ) );
		}

		$post = get_post( $post_id );

		if ( ! $post ) {
			wp_send_json_error( __( 'Post not found.', 'images-sync-for-cloudflare' ) );
		}
		...
		if ( $resolved->is_empty() ) {
			...
			$result['upload_reason'] = __( 'Source image not found or empty.', 'images-sync-for-cloudflare' );
			wp_send_json_success( $result );
		}
		...
		if ( $att_cf_id !== '' && ! \CFI\Core\Signature::has_changed( $file_path, $att_sig ) ) {
			...
			if ( \CFI\Core\Signature::has_changed( $file_path, $stored_sig ) ) {
				...
				$result['upload_reason'] = __( 'File unchanged since last sync.', 'images-sync-for-cloudflare' );
			}
		} else {
			$result['status']        = 'skip_disabled';
			$result['upload_reason'] = __( 'Upload not needed or behavior not enabled.', 'images-sync-for-cloudflare' );
		}
	...
	private function render_form( ?array $mapping ): void {
		...
		wp_localize_script(
			...
			array(
				...
				'sourceKeyConfig' => array(
					'acf_field'               => array(
						'label'       => __( 'ACF Field Name', 'images-sync-for-cloudflare' ),
						'placeholder' => 'hero_image',
						'required'    => true,
					),
					'post_meta_attachment_id'  => array(
						'label'       => __( 'Meta Key', 'images-sync-for-cloudflare' ),
				...
				'i18n'            => array(
					'required'      => __( 'This field is required.', 'images-sync-for-cloudflare' ),
					'invalidKey'    => __( 'Only letters, numbers, underscores, hyphens, colons, and dots are allowed.', 'images-sync-for-cloudflare' ),
					'keyTooLong'    => __( 'Maximum 191 characters.', 'images-sync-for-cloudflare' ),
					'selectPostType' => __( 'Please select a post type.', 'images-sync-for-cloudflare' ),
				),
			)
		);
		...
		<form method="post" id="cfi-mapping-form" novalidate>
				...
				<table class="form-table">
					<tr>
						...
						<td>
							...
							<p class="description">
								<?php esc_html_e( 'Choose which content type this mapping applies to.', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
					<tr>
						<th>
							<label for="status">
								<?php esc_html_e( 'Post Status', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
							<select id="status" name="status">
								<option value="any" <?php selected( $m['status'], 'any' ); ?>>
									<?php esc_html_e( 'Any', 'images-sync-for-cloudflare' ); ?>
								</option>
								<option value="publish" <?php selected( $m['status'], 'publish' ); ?>>
									<?php esc_html_e( 'Published only', 'images-sync-for-cloudflare' ); ?>
								</option>
							</select>
							<p class="description">
								<?php esc_html_e( 'Sync only published posts, or include drafts and other statuses.', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
					<tr>
						<th>
							<label for="source_type">
								<?php esc_html_e( 'Source Type', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
								...
								<?php foreach ( $source_types as $st ) : ?>
									...
									if ( 'acf_field' === $st && ! $has_acf ) :
										...
										<option value="<?php echo esc_attr( $st ); ?>" disabled>
											...
											echo esc_html( sprintf( __( '%s (ACF not active)', 'images-sync-for-cloudflare' ), $label ) );
											?>
										</option>
									<?php else : ?>
							...
							<p class="description">
								<?php esc_html_e( 'Where does the original image come from?', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
			...
			<div class="cfi-form-section">
				...
				<table class="form-table">
					<tr>
						<th>
							<label for="target_url_meta">
								<?php esc_html_e( 'Delivery URL Meta Key', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
							...
							<p class="description">
								<?php esc_html_e( 'The post meta key where the Cloudflare delivery URL will be stored. Use this key in your theme to display the optimized image.', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
					<tr>
						<th>
							<label for="target_id_meta">
								<?php esc_html_e( 'CF Image ID Meta Key', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
							<div class="cfi-input-with-copy">
							...
							<p class="description">
								<?php esc_html_e( 'Optional. Stores the Cloudflare image ID for management purposes.', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
					<tr>
						<th>
							<label for="target_sig_meta">
								<?php esc_html_e( 'Change Signature Meta Key', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
							<div class="cfi-input-with-copy">
							...
							<p class="description">
								<?php esc_html_e( 'Optional. Stores a hash to detect image changes and avoid redundant uploads.', 'images-sync-for-cloudflare' ); ?>
							</p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th>
							<label for="preset_id">
								<?php esc_html_e( 'Preset', 'images-sync-for-cloudflare' ); ?>
							</label>
						</th>
						<td>
							<select id="preset_id" name="preset_id">
								<option value="">
									<?php esc_html_e( '— Default (public) —', 'images-sync-for-cloudflare' ); ?>
								</option>
							...
							<p class="description">
								...
								echo wp_kses(
									sprintf(
										...
										__( 'Image variant preset for the delivery URL. Manage presets under %s.', 'images-sync-for-cloudflare' ),
										$presets_link
									),
			...
			<div class="cfi-form-section">
				...
				<table class="form-table">
					<tr>
						<th><?php esc_html_e( 'Auto-Sync Triggers', 'images-sync-for-cloudflare' ); ?></th>
						<td>
							<fieldset>
								<label>
									<input type="checkbox" name="trigger_save_post" value="1" <?php checked( $m['triggers']['save_post'] ?? true ); ?> />
									<?php esc_html_e( 'When a post is saved', 'images-sync-for-cloudflare' ); ?>
								</label>
								<br/>
								<label>
									<input type="checkbox" name="trigger_acf_save_post" value="1" <?php checked( $m['triggers']['acf_save_post'] ?? true ); ?> <?php disabled( ! $has_acf ); ?> />
									<?php esc_html_e( 'When ACF fields are updated', 'images-sync-for-cloudflare' ); ?>
								</label>
								<?php if ( ! $has_acf ) : ?>
									<p class="description">
										<?php esc_html_e( 'ACF is not active on this site. Install Advanced Custom Fields to enable this trigger.', 'images-sync-for-cloudflare' ); ?>
									</p>
								<?php endif; ?>
							</fieldset>
						</td>
					...
					<tr>
						<th><?php esc_html_e( 'Upload Behavior', 'images-sync-for-cloudflare' ); ?></th>
						<td>
							<fieldset>
								<label>
									<input type="checkbox" name="upload_if_missing" value="1" <?php checked( $m['behavior']['upload_if_missing'] ?? true ); ?> />
									<?php esc_html_e( 'Upload image if not yet on Cloudflare', 'images-sync-for-cloudflare' ); ?>
								</label>
								<br/>
								<label>
									<input type="checkbox" name="reupload_if_changed" value="1" <?php checked( $m['behavior']['reupload_if_changed'] ?? true ); ?> />
									<?php esc_html_e( 'Re-upload if the local file has changed', 'images-sync-for-cloudflare' ); ?>
								</label>
								<br/>
								<label>
									<input type="checkbox" name="clear_on_empty" value="1" <?php checked( $m['behavior']['clear_on_empty'] ?? true ); ?> />
									<?php esc_html_e( 'Clear delivery URL if the source image is removed', 'images-sync-for-cloudflare' ); ?>
								</label>
								<br/>
								<label>
									<input type="checkbox" name="store_cf_id_on_post" value="1" <?php checked( $m['behavior']['store_cf_id_on_post'] ?? true ); ?> />
									<?php esc_html_e( 'Store Cloudflare image ID on the post', 'images-sync-for-cloudflare' ); ?>
								</label>
								<br/>
								<label>
									<input type="checkbox" name="delete_cf_on_reupload" value="1" <?php checked( $m['behavior']['delete_cf_on_reupload'] ?? false ); ?> />
									<?php esc_html_e( 'Delete old Cloudflare image when re-uploading', 'images-sync-for-cloudflare' ); ?>
								</label>
								<p class="description">
									<?php esc_html_e( 'Disabled by default. Enable only if you are sure no other mapping or post references the same Cloudflare image.', 'images-sync-for-cloudflare' ); ?>
								</p>
							</fieldset>
						</td>
					</tr>
	...
	private function render_table( array $mappings ): void {
		if ( empty( $mappings ) ) {
			echo '<p>' . esc_html__( 'No mappings yet. Click "Add New" to create one.', 'images-sync-for-cloudflare' ) . '</p>';
			return;
		}
		...
		foreach ( $mappings as $map ) :
			$preset_name = __( 'Default (public)', 'images-sync-for-cloudflare' );
			if ( ! empty( $map['preset_id'] ) ) {
				$p           = $this->presets->find( $map['preset_id'] );
				$preset_name = $p ? $p['name'] : __( '(deleted)', 'images-sync-for-cloudflare' );
			}
